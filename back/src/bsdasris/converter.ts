  ...
  return {
    ...
    destination: nullIfNoValues<BsdasriDestination>({
      ...
      reception: nullIfNoValues<BsdasriReception>({
        ...
        packagingInfos: bsdasri.destinationWastePackagingsInfo as BsdasriPackagingInfo[],
...
export const unflattenGroupingDasri = (dasri: Bsdasri): InitialBsdasri => ({
  ...
  quantity: countWasteQuantity(dasri.destinationWastePackagingsInfo),
...
function flattenEmissionInput(input: { emission?: BsdasriEmissionInput }) {
  ...
  const emitterWastePackagingsInfo = chain(
    input.emission,
    e => e.packagingInfos
  );
...
function flattenTransportInput(input: { transport?: BsdasriTransportInput }) {
  ...
  const transporterWastePackagingsInfo = chain(
    input.transport,
    t => t.packagingInfos
  );
  return {
    ...
    transporterWasteRefusedWeightValue: chain(input.transport, t =>
      chain(t.wasteAcceptation, w => w.refusedWeight)
    ),
...
function flattenReceptiontInput(input: { reception?: BsdasriReceptionInput }) {
  ...
  const destinationWastePackagingsInfo = chain(
    input.reception,
    r => r.packagingInfos
  );
  return {
...
export function flattenBsdasriInput(
  formInput: Pick<
    BsdasriInput,
    "waste" | "emitter" | "transporter" | "destination"
  >
